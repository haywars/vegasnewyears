...
// LATITUDE STANDARD DEVIATION STUFF
foreach ($venues as $key=>$v) {	
	$lat_sum += $v['latitude']; 
	//echo $v['latitude'].'<br />';
	}
$lat_average = $lat_sum/sizeof($venues);
foreach ($venues as $key=>$v) {	$lat_diff_sum += pow(($v['latitude']-$lat_average),2); }
$lat_dev =  sqrt($lat_diff_sum/sizeof($venues));
$lat_high = $lat_average + $lat_dev*2;
$lat_low = $lat_average - $lat_dev*2;
// LONGITUDE STANDARD DEVIATION STUFF
foreach ($venues as $key=>$v) {	$lon_sum += $v['longitude']; }
$lon_average = $lon_sum/sizeof($venues);
foreach ($venues as $key=>$v) {	$lon_diff_sum += pow(($v['longitude']-$lon_average),2); }
$lon_dev =  sqrt($lon_diff_sum/sizeof($venues));
$lon_high = $lon_average + $lon_dev*2;
$lon_low = $lon_average - $lon_dev*2;
...
<?
foreach ($venues as $key=>$v) {
	//check is it's not an outlier
	if($v['latitude'] < $lat_high && $v['latitude'] > $lat_low && $v['longitude'] < $lon_high && $v['longitude'] > $lon_low) {
